...
final class Commands {
    ...
    static class Home extends BaseCommand {
        ...
        private final Pattern LOCATION_NAME_REGEX = Pattern.compile("[^a-zA-Z0-9]");
        ...
        public void onSetHome(Player player, String nome) {
            ...
            nome = LOCATION_NAME_REGEX.matcher(nome).replaceAll("").toLowerCase();
            if (nome.length() > 16) {
            ...
            MessageOptions compassOption = new MessageOptions(false, nome);
            meta.displayName(EterniaLib.getChatCommons().parseMessage(Messages.HOME_ITEM_NAME, compassOption));
            ...
            EterniaLib.getChatCommons().sendMessage(player, Messages.HOME_LIMIT_REACHED, options);
        }
